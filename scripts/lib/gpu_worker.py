...
class GPUWorker:
  ...
  def render(task: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cmd = [
      ...
      "--render_num_samples",
      str(task["render_config"]["samples"]),
    ...
    constraints = GPUWorker._extract_constraints(
      render_output, prefix, task["tau"], worker_id
    )
  ...
  def _extract_constraints(
    ...
    try:
      ...
      logger.warning(
        f"Worker {worker_id}: "
        "Cannot import constraint extractor"
      )
      return {}

    scenes_file = output_dir / f"{prefix}_scenes.json"
    if not scenes_file.exists():
      logger.warning(
        f"Worker {worker_id}: Scene file not found"
      )
      return {}
    ...
    for scene in scenes:
      ...
      try:
        ...
        logger.warning(
          f"Worker {worker_id}: "
          f"Constraint extraction failed "
          f"for {scene_id}: {e}"
        )
    ...
    logger.info(
      f"Worker {worker_id}: "
      f"Extracted {len(constraints)} constraints"
    )
    return constraints
